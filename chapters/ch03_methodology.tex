...
In the outward sweep, the accelerations to the constrained motion is computed (line \ref{23} and \ref{24})~\cite{shakhimardanov2015composable}. The obtained \textit{constraint force magnitudes} in the previous step are then substituted in the equation given in line \ref{23}. Here, $\ddot{q}_{i+1}$ represents accelerations at the joint $i+1$.
